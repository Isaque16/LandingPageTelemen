...
tipoMensagem.forEach(tipo => {
    tipo.addEventListener('change', () => {
        ...
        if (tipo.value == 'aovivo')
            numForm = 0; // Número do form ao vivo
        else if (tipo.value == 'portelefone')
            numForm = 1; // Número do form por telefone
        updateForm(numForm);
    });
});
...
submitbtn.addEventListener('click', () => {
    ...
    if (verifyInput(isValid))
        popupInputData(); // Função que mostra os dados passado nos inputs
});
// Botão para confirmar e enviar os dados para o servidor
const cancelBtn = document.getElementById('cancelar-agendamento');
